...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug  7 09:29:24 2018
...
from tensorflow.python.keras.layers import Input,Conv2D,UpSampling2D,Dense,Reshape,BatchNormalization
...
# Creat Model
def build_model_VGG16AE_SPARSE(dim,lr,regularizer,TRAINABLE=False):
    """
    Arguments:
        dim(int) : the dimension of latent space
        lr(float) : the learning rate for training
        regularizer(float): the parameter of activity_regularizer
    ...
    encoder = BatchNormalization(name = 'bc1')(encoder)
    encoder = base_model.get_layer('block1_conv2')(encoder)
    encoder = BatchNormalization(name = 'bc2')(encoder)
    encoder = base_model.get_layer('block1_pool')(encoder)
    
    # block2
    encoder = base_model.get_layer('block2_conv1')(encoder)
    encoder = BatchNormalization(name = 'bc3')(encoder)
    encoder = base_model.get_layer('block2_conv2')(encoder)
    encoder = BatchNormalization(name = 'bc4')(encoder)
    encoder = base_model.get_layer('block2_pool')(encoder)
    
    # block3
    encoder = base_model.get_layer('block3_conv1')(encoder)
    encoder = BatchNormalization(name = 'bc5')(encoder)
    encoder = base_model.get_layer('block3_conv2')(encoder)
    encoder = BatchNormalization(name = 'bc6')(encoder)
    encoder = base_model.get_layer('block3_conv3')(encoder) 
    encoder = BatchNormalization(name = 'bc7')(encoder)
    encoder = base_model.get_layer('block3_pool')(encoder)
    
    # block4
    encoder = base_model.get_layer('block4_conv1')(encoder)
    encoder = BatchNormalization(name = 'bc8')(encoder)
    encoder = base_model.get_layer('block4_conv2')(encoder)
    encoder = BatchNormalization(name = 'bc9')(encoder)
    encoder = base_model.get_layer('block4_conv3')(encoder)
    encoder = BatchNormalization(name = 'bc10')(encoder)    
    encoder = base_model.get_layer('block4_pool')(encoder)
    
    # block5
    encoder = base_model.get_layer('block5_conv1')(encoder)
    encoder = BatchNormalization(name = 'bc11')(encoder)
    encoder = base_model.get_layer('block5_conv2')(encoder)
    encoder = BatchNormalization(name = 'bc12')(encoder)
    encoder = base_model.get_layer('block5_conv3')(encoder)
    encoder = BatchNormalization(name = 'bc13')(encoder)
    encoder = base_model.get_layer('block5_pool')(encoder)     
    
    # block6
    encoder = base_model.get_layer('flatten')(encoder)
    encoder = base_model.get_layer('fc1')(encoder)
    encoder = BatchNormalization(name = 'bc14')(encoder)
    encoder = base_model.get_layer('fc2')(encoder)
    encoder = BatchNormalization(name = 'bc15')(encoder)
   
    #--------latent space (trainable) ------------
    encoder = Dense(dim, activation='relu',activity_regularizer=regularizers.l1(regularizer), name = 'encoder_output')(encoder)     
    #activity_regularizer=regularizers.l1(10e-8),10e-6,10e-5
    #--------------decoder (trainable)-----------   
    # Block 6
    decoder = Dense(4096, activation='relu', name = 'dblock_dense1')(encoder)
    decoder = BatchNormalization(name = 'dbc1')(decoder)
    decoder = Dense(4096, activation='relu', name = 'dblock_dense2')(decoder)
    decoder = BatchNormalization(name = 'dbc2')(decoder)
    decoder = Dense(7*7*512, activation='relu', name = 'dblock_dense3')(decoder)
    decoder = BatchNormalization(name = 'dbc3')(decoder)
    decoder = Reshape([7,7,512],name = 'dblock_reshpe')(decoder)    
    decoder = UpSampling2D((2,2),name = 'dblock_ups1')(decoder)        
    
    # Block 5
    decoder = Conv2D(512, (3, 3), activation='relu', padding='same', name='dblock_conv1')(decoder)
    decoder = BatchNormalization(name = 'dbc4')(decoder)
    decoder = UpSampling2D((2,2),name = 'dblock_ups2')(decoder)
    
    # Block 4
    decoder = Conv2D(256, (3, 3), activation='relu', padding='same', name='dblock_conv2')(decoder)
    decoder = BatchNormalization(name = 'dbc5')(decoder)
    decoder = UpSampling2D((2,2),name = 'dblock_ups3')(decoder)     
         
    # Block 3
    decoder = Conv2D(128, (3, 3), activation='relu', padding='same', name='dblock_conv3')(decoder)
    decoder = BatchNormalization(name = 'dbc6')(decoder)
    decoder = UpSampling2D((2,2),name = 'dblock_ups4')(decoder)        
     
    # Block 2
    decoder = Conv2D(64, (3, 3), activation='relu', padding='same', name='dblock_conv4')(decoder)
    decoder = BatchNormalization(name = 'dbc7')(decoder)
    decoder = UpSampling2D((2,2),name = 'dblock_ups5')(decoder) 
    
    # Block 1
    decoder = Conv2D(3, (3, 3), activation='sigmoid', padding='same', name='dblock_conv5')(decoder)
...
if __name__ == '__main__':
    ...
    help_ = "parameter for regularizer"
    parser.add_argument("-re", "--regularizer", type=float, default=15e-7, help=help_)
    ...
    if args.model:
        # load model trained
        print('load model')
        ae_sparse = load_model(args.model)
        print('finish loading model')
        # test model
        test_model(ae_sparse,x_test)
        #tsne
        tsne(ae_sparse,x_train)
    ...
    if args.path: 
        #train model
        ae_sparse = build_model_VGG16AE_SPARSE(args.dimension,args.rate,args.regularizer)
        # split training set and validation set
        x_train, x_valid = train_test_split(x_train, test_size=0.2, random_state=123)
        history = ae_sparse.fit(x_train, x_train, batch_size=32,
                          epochs=args.epoch,verbose=1,validation_data = (x_valid,x_valid))
        #save model
        ae_sparse.save(args.path)
#        ae_sparse.save_weights('./save_model/ae_sparse_weights.h5', by_name=True)
    
        #evaluate model
        evaluate_model(history, ae_sparse, x_test)
        #test model
        test_model(ae_sparse,x_test)
        #tsne
        tsne(ae_sparse,x_train)
